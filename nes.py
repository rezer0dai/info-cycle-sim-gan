...
class NoisyLinear(nn.Linear):
    def __init__(self, in_features, out_features, device):
        ...
        self.register_buffer('noise', torch.zeros(out_features, in_features).to(self.device))
    ...
    def sample_noise(self):
        torch.randn(self.noise.size(), out=self.noise).to(self.device)

    def remove_noise(self):
        torch.zeros(self.noise.size(), out=self.noise).to(self.device)
...
class NoisyNet(nn.Module):
    def __init__(self, layers, device):
        ...
        for i, layer in enumerate(self.layers):
            self.sigma.append(
                nn.Parameter(torch.Tensor(layer.out_features, layer.in_features).fill_(.017).to(self.device)))
    ...
    def forward(self, data):
        for i, layer in enumerate(self.layers[:-1]):
            #  data = F.leaky_relu(layer(self.sigma[i], data), .2)
            data = F.relu(layer(self.sigma[i], data))
        return self.layers[-1](self.sigma[-1], data)

class NoisyNetFactory:
    def __init__(self, layers, device):
        self.net = [ NoisyLinear(layer, layers[i+1], device) for i, layer in enumerate(layers[:-1]) ]

    def head(self):
        return NoisyNet(self.net, self.net[0].device)
